...
def on_start_response(response, results):
    ...
    global session
    empty_dict = dbus.Dictionary(signature="sv")
    portal.NotifyKeyboardKeycode(session, empty_dict, 125, 1,
                                 dbus_interface=remote_desktop_iface)
    portal.NotifyKeyboardKeycode(session, empty_dict, 125, 0,
                                 dbus_interface=remote_desktop_iface)
...
def on_select_devices_response(response, results):
    if response != 0:
        print("Failed to select devices: %d"%response)
        terminate()
        return
